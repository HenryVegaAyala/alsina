    ...
    function Impresion($informacion)
    {
        ...
        while ($row = $resultado->read()) {
            ...
            if (strlen($row['DESC_CORTAR']) > 35):
                $this->MultiCell(0.6, 0.90, $i, 1, '', 'C');
                $this->MultiCell(1.5, 0.90, utf8_decode(strtoupper($row['NUM_PROD'])), 1, '', 'L');
                $this->MultiCell(6, 0.90, utf8_decode(strtoupper($row['DESC_CORTAR'])), 1, 'L');
                $this->MultiCell(1.4, 0.90, utf8_decode(strtoupper($row['PREC_X_DIA'])), 1, '', 'C');
                $this->MultiCell(1.4, 0.90, utf8_decode(strtoupper($row['PESO_REAL'])), 1, '', 'C');
                $this->MultiCell(1.4, 0.90, utf8_decode(strtoupper($row['PESO_VOL'])), 1, '', 'C');
                $this->MultiCell(1, 0.90, utf8_decode(strtoupper($row['UD'])), 1, '', 'C');
                $this->MultiCell(1.4, 0.90, utf8_decode(strtoupper($row['PESO_REAL_TOTAL'])), 1, '', 'C');
                $this->MultiCell(1.4, 0.90, utf8_decode(strtoupper($row['CANT_DIAS'])), 1, '', 'C');
                $this->MultiCell(1.4, 0.90, utf8_decode(strtoupper($row['COST_TOTAL'])), 1, '', 'C');
                $this->MultiCell(1.4, 0.90, utf8_decode(strtoupper($row['PESO_V_TOTAL'])), 1, '', 'C');
                $i++;
                $this->Ln();
            else:
